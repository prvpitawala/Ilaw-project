        ...
        this.main ={
                ...
                return `
                    <div class="collection-document-upload-section-container">
                        <div class="collection-document-upload-container">
                            <div class="back-button-collection" id="backButton-collection" data="collection-document-update-section">
                                <img src="" class="back-button-img" alt="back">
                            </div>
                            <br>
                            <div class="fileupbox" >
                                <div class="label">Pick Files</div>
                                <div class="file-upload-box" id='CollectionUploadBox'>
                                    <div class="plus-icon">+</div>
                                    <div class="file-text" id="fileNamesText">Drag the files or Paste the files</div>
                                    <input type="file" id="fileInputCollection" style="display: none;" accept=".txt,.pdf,.docx,.odt" multiple>
                                </div>
                            ...
                            <div>
                                ...
                                <input class="collection-name-input" type="text" id="input-collection-name" placeholder="Type your new collection name.">
                            </div>
                            <button id="collectionUploadButton" class="upload-collection-button" data="collection-document-update-section">Update</button>
                        </div>     
                    </div>
                `
            },
            style: function() {
                return `
                    ...
                    .collection-document-upload-section-container {
                        display: flex;
                    ...
                    .collection-document-upload-container {
                        display: flex;
                        flex-direction: column;
                        align-items: stretch;
                    }

                    .back-button-collection {
                        display: flex;
                        width: 20px;
                        height: 20px;
                        margin-left: 0px;
            ...
            event: function() {
                return [
                    {
                        element: document.querySelector('#CollectionUploadBox'),
                        eventType: 'click',
                        eventGroup: 'collectionAdd',
                        handler: function(event) {
                            event.stopImmediatePropagation();
                            const collectionInputElement = document.querySelector('#fileInputCollection');
                            collectionInputElement.click();
                        }
                    },
                    {
                    ...
                    },
                    {
                        ...
                        handler: function(event) {
                            event.preventDefault();
                            const fileUploadBox = document.querySelector('.file-upload-box');
                            const fileInput = document.getElementById('fileInputCollection');
                    ...
                    {
                        element: document.getElementById('backButton-collection'),
                        eventType: 'click',
                        eventGroup: 'collectionAdd',
                        handler: function() {
                    ...
                    {
                        element: document.querySelector('#input-collection-name'),
                        eventType: 'keyup',
                        eventGroup: 'collectionAdd',
                        handler: function() {
                            const collectionNameElement = document.querySelector('#input-collection-name');
                            if (collectionNameElement.value) {
                                if (halperFunctions.collectionList.includes(collectionNameElement.value)) {
                                    collectionNameElement.style.borderColor = '#c53a3a';
                                } else {
                                    collectionNameElement.style.borderColor = '#67ac5a';
                                }
                            } else {
                                collectionNameElement.style.borderColor = '';
                            }
                        }
                    },
                    {
                        element: document.querySelector('#collectionUploadButton'),
                        eventType: 'click',
                        eventGroup: 'collectionAdd',
                        handler: function(event) {
                            const collectionNameElement = document.querySelector('#input-collection-name');
                            ...
                            return halperFunctions.newCollectionUpdate();
                        }
                    },
                    {
                        element: document,
            ...
            navigator: function(options) {
                ...
                halperFunctions.themeInit();
                halperFunctions.collectionList = options.collectionList;
                const fileNamesText = document.getElementById('fileNamesText');
                fileNamesText.innerHTML = 'Drag the files or Paste the files';
                const collectionNameEle = document.getElementById('input-collection-name');
                collectionNameEle.value = null;
            }
        }
        ...
        const halperFunctions = {
            ...
            themeInit: function() {
                if (document.documentElement.getAttribute('data-theme') === 'dark') {
                    document.querySelector('#backButton-collection').querySelector('img').setAttribute('src','../public/icons/chevron-left-50-ffffff.png')
                } else {
                    document.querySelector('#backButton-collection').querySelector('img').setAttribute('src','../public/icons/chevron-left-50-1a1a1a.png')
                }
            },
            ...
            // newCollectionUpdate: async function() {
            //     const fileInputElement = document.getElementById('fileInputCollection');
            //     const collectionNameElement = document.querySelector('#input-collection-name');
            //     const newCollectionName = collectionNameElement.value;

            //     if (newCollectionName) {
            //         if (halperFunctions.collectionList.includes(newCollectionName)) {
            //             notificationManager.warning('this Collection Name is already excist');
            //             return;
            //         }
            //     } else {
            ...
            //     try {
            //         const response = await fetch(`http://127.0.0.1:5000/api/collections/${newCollectionName}`, {
            //             headers: {
            ...
            //             notificationManager.success('Successfully added Collection');
            //             if (uiManager.currentView === 'collection-add-card') {
            //                 uiManager.navigateToView('dashboard','dashboard');
            //             }
            //         } else {
            ...
            //         notificationManager.error('Have some error ');
            //     }
            //     // api.createCollection(newCollectionName, fileData, "token Raveen2244");
            // }, 


            // Updated frontend code with progress tracking
            newCollectionUpdate: async function() {
                const fileInputElement = document.getElementById('fileInputCollection');
                const collectionNameElement = document.querySelector('#input-collection-name');
                const newCollectionName = collectionNameElement.value;
                ...
                if (newCollectionName) {
                    if (halperFunctions.collectionList.includes(newCollectionName)) {
                        notificationManager.warning('this Collection Name is already exist');
                        return;
                    }
                } else {
                    notificationManager.warning('Collection Name is needed');
                ...
                // File extension validation (same as before)
                const allowedExtensions = ['pdf', 'txt', 'docx', 'odt'];
                ...
                try {
                    // First, upload files and start processing
                    const response = await fetch(`http://127.0.0.1:5000/api/collections/${newCollectionName}`, {
                        headers: {
                            Authorization: 'Bearer Raveen2244'
                        },
                    ...
                    this.listenToProgress(taskId, newCollectionName);
            ...
            listenToProgress: function(taskId, collectionName) {
                const eventSource = new EventSource(`http://127.0.0.1:5000/api/collections/${collectionName}/progress/${taskId}`);
